...
def plot_line(radius, length, center_x, center_y, offset_alpha):
    """
    ...
    offset_alpha_rad = np.radians(offset_alpha)
    theta_rad = offset_alpha_rad
    line_x_rotatet = line_x * np.cos(theta_rad) - line_y * np.sin(theta_rad)
    line_y_rotatet = line_x * np.sin(theta_rad) + line_y * np.cos(theta_rad)
...
def plot_graph_with_semi_circle(df):
    ...
    for index, row in df.iterrows():
        ...
        if(row['alpha'] == 0):
            plot_line(row['radius'], row['radian'], center_x, center_y, offset_alpha)
...
#plot_semi_circle( 5, 45, 0, 0, 0)
#plot_semi_circle(5, -45, 10, 0, 180)
#plot_line(5, 5, 0, 0, 45)

center_x = 0
center_y = 0        
center_x += -10 * np.sin(np.radians(180))
center_y += 10 * np.cos(np.radians(180))
